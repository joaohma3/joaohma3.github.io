<?php
    ...
    // Chamada de todas as Classes DAO do Projeto!
    require_once './DAO/MovimentoDAO.php';
    require_once './DAO/CategoriaDAO.php';
    require_once './DAO/EmpresaDAO.php';
    require_once './DAO/ContaDAO.php';

    // Objetos de todas as Classes chamadas!
    $objCategoria = new CategoriaDAO();
    $objEmpresa = new EmpresaDAO();
    $objConta = new ContaDAO();

    // Variáveis Globais (Selects)!
    $tipo = '';
    $categoria = '';
    $empresa = '';
    $conta = '';
    ...
    // Arrays de todas as consultas de acordo a sua Classe DAO!
                ...
                <form action="realizar_movimento.php" method="POST">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input class="form-control" placeholder="Digite o Valor (R$) aqui..." name="valor" id="valor" value="<?= isset($valor) ? $valor : '' ?>"/>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" name="categoria" id="categoria">
                                ...
                                <?php foreach($categorias as $ct): ?>
                                    <option value="<?= $ct['id_categoria'] ?>" <?= $categoria == 1 ? 'selected' : '' ?>><?= $ct['nome_categoria'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="empresa" id="empresa">
                                ...
                                <?php foreach($empresas as $emp): ?>
                                    <option value="<?= $emp['id_empresa'] ?>" <?= $empresas == 1 ? 'selected' : '' ?>><?= $emp['nome_empresa'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" name="conta" id="conta">
                                ...
                                <?php foreach($contas as $cnt): ?>
                                    <option value="<?= $cnt['id_conta'] ?>" <?= $contas == 1 ? 'selected' : '' ?>><?= $cnt['banco_conta'] ?> - R$ <?= number_format($cnt['saldo_conta'], 2, ',', '.') ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="alignBtn">
                        <button type="submit" class="btn btn-success" name="btnRealizar" onclick="return ValidarMovimentos();">Realizar Movimento</button>
                    </div>
                </form>
